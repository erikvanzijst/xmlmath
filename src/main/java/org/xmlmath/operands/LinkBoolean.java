...
import org.xmlmath.x10.LinkBooleanType;
import org.xmlmath.x10.ValueType;

/**
 * Convenience link to declarations of type <tt>Boolean</tt>.
 *
 * @author	Erik van Zijst - erik@prutser.cx
 * @version	v1.1, 18.apr.2006
 */
public class LinkBoolean extends AbstractBooleanValue {

	private Declaration booleanDeclaration = null;
	
	@Override
	public Boolean getBoolean() {
		try {
			return (Boolean)booleanDeclaration.getValue();
		} catch(ClassCastException cce) {
			throw new TypeCastException("Incompatible cast from " +
					cce.getMessage() + " to Boolean.");
		}
	}
	...
	protected void build(ValueType xmlObject) throws ParseException {
		...
		LinkBooleanType xml = (LinkBooleanType)xmlObject;
		
		booleanDeclaration = Scope.getScope().getDeclaration(xml.getName());
